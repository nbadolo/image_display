...
#%%        
    ...
    fdir_star = fdir + 'star/both/' 
    fdir_psf = fdir +'psf/both/'
    ...
    for j in range(n_lst_fltr3):
      ...
      # """
        ...
        for i in range (nFrames):
              ...
              sub_v_arr[i]=sub_v
              if np.any(np.min(sub_v_arr[i])<= 0): #i==3 or i==7:
        ...
        for i in range (nFrames):   
              plt.subplot(3,4,i+1)
              if i != 2 and i != 5 and i != 3 and i != 7:
                  if np.any(np.min(sub_v_arr[i])<= 0):
              ...
              if i == 2:
                  if np.any(np.min(sub_v_arr[i-1])<= 0):
                  
                    plt.imshow(sub_v_arr[i-1], cmap ='inferno', origin='lower',vmin=Vmin[1], 
                    ...
                    plt.text(-17*size[0]//25., 3*size[1]//2, im_name_lst[3], color='cyan',
                     ...
                     plt.imshow(np.log10(sub_v_arr[i-1]), cmap ='inferno', origin='lower',vmin=Vmin[1], 
                                 vmax=Vmax[1], extent = [x_min , x_max, y_min , y_max])   
                     plt.colorbar(label='ADU in log$_{10}$ scale')       
                     q = plt.quiver(X[::X_step,::X_step],Y[::X_step,::X_step],U[::X_step,::X_step], V[::X_step,::X_step])
                     plt.quiverkey(q, X = -0.05, Y = 1.03, U = 0.03, label='pol. degree vector norm scale 0.03 ', labelpos='E')
                  ...
                  if i == 5:
                     if np.any(np.min(sub_v_arr[4]) <= 0): 
                         
                        plt.imshow(sub_v_arr[4], cmap ='inferno', origin='lower',vmin=Vmin[1], 
                                    vmax=Vmax[1], extent = [x_min , x_max, y_min , y_max])   
                        plt.colorbar(label='ADU in log$_{10}$ scale')       
                        q_ = plt.quiver(X[::X_step,::X_step],Y[::X_step,::X_step],U_[::X_step,::X_step], V_[::X_step,::X_step])
                        plt.quiverkey(q_, X = -0.05, Y = 1.03, U_ = 0.03, label='pol. degree vector norm scale 0.03 ', labelpos='E')
                        plt.text(-17*size[0]//25., 3*size[1]//2, im_name_lst[3], color='cyan',
                                 fontsize='x-small', ha='center')
                     else :
                         plt.imshow(sub_v_arr[4], cmap ='inferno', origin='lower',vmin=Vmin[1], 
